...
import rsl_rl
from rsl_rl.algorithms import PPO, Distillation, RL2PPO, DEBUGPPO
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] == "PPO" or self.alg_cfg["class_name"] == "RL2PPO" or self.alg_cfg["class_name"] == "DEBUGPPO":
        ...
        self.alg: PPO | Distillation | RL2PPO | DEBUGPPO = alg_class(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # zero-init prev_action for RL^2
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    # Reset prev_actions at episode boundary
                    if isinstance(self.alg, RL2PPO):
                        prev_actions[dones] = torch.zeros(self.env.action_space.shape, device=self.device)
                    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                ...
                if self.training_type == "rl":
                    if isinstance(self.alg, RL2PPO):
                        self.alg.compute_returns(obs, prev_actions)
                    else:
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        for key, value in locs["loss_dict"].items():
            self.writer.add_scalar(f"Loss/{key}", value, self.tot_timesteps)
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            if self.alg.rnd:
                self.writer.add_scalar("Rnd/mean_extrinsic_reward", statistics.mean(locs["erewbuffer"]), self.tot_timesteps)
                self.writer.add_scalar("Rnd/mean_intrinsic_reward", statistics.mean(locs["irewbuffer"]), self.tot_timesteps)
    ...
    # 旧版
    def log_backup(self, locs: dict, width: int = 80, pad: int = 35):
        # Compute the collection size
        collection_size = self.num_steps_per_env * self.env.num_envs * self.gpu_world_size
        # Update total time-steps and time
        self.tot_timesteps += collection_size
